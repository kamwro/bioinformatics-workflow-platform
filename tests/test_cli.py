...
def test_start_workflow_runs_end_to_end(
    ...
    def fake_register_local_run(**kwargs: Any) -> dict[str, Any]:
        calls["register"] = kwargs
        return {
            "id": "run-123",
            "run_name": kwargs["run_name"],
            "status": "COMPLETED",
            "multiqc_report_path": "results/multiqc/multiqc_report.html",
        }

    monkeypatch.setattr(cli, "run_nextflow_qc", fake_run_nextflow_qc)
    monkeypatch.setattr(cli, "register_local_run", fake_register_local_run)
    ...
    assert "Registered run in BioQC Portal" in printed
